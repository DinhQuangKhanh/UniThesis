...
import { studentGroupService, OpenGroupDto } from '@/lib/studentGroupService'
...
export function StudentOpenGroupsPage() {
    ...
    const fetchOpenGroups = async () => {
        try {
            setLoading(true)
            setError(null)
            const groups = await studentGroupService.getOpenGroups()
                ...
                {loading ? (
                    ...
                    <div className="max-w-2xl mx-auto">
                        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-8 text-center">
                            ...
                            <p className="text-gray-500">Hiện tại không có nhóm mới để tham gia. Hãy kiểm tra lại sau hoặc tạo nhóm của riêng bạn.</p>
                        </div>
                    </div>
                ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {openGroups.map((group) => (
                            <div key={group.groupId} className="bg-white rounded-lg shadow-sm border border-gray-200 hover:shadow-md transition p-6">
                                <div className="mb-4">
