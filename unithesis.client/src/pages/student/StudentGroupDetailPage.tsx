...
import { studentGroupService, StudentGroupDto, JoinRequestDto } from '@/lib/studentGroupService'
                    ...
                    <div className="max-w-4xl mx-auto space-y-6">
                        ...
                        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                            <div className="flex items-start justify-between mb-6">
                                ...
                                <span className={`px-4 py-2 rounded-full text-sm font-semibold ${myGroup.isOpenForRequests ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>
                            ...
                            <div className="grid grid-cols-2 gap-4 mb-6 p-4 bg-gray-50 rounded-lg">
                                <div>
                                    ...
                                    <p className="text-2xl font-bold text-gray-800">{(myGroup.members?.length ?? 0)}/{myGroup.maxMembers}</p>
                                </div>
                                <div>
                                    <p className="text-sm text-gray-600">Ngày tạo</p>
                                    <p className="text-lg font-semibold text-gray-800">{new Date(myGroup.createdAt).toLocaleDateString('vi-VN')}</p>
                                </div>
                            </div>
                            ...
                            <div className="space-y-3">
                                {myGroup.members && myGroup.members.length > 0 ? (
                                    myGroup.members.map((member) => (
                                        <div key={member.studentId} className="flex items-center justify-between p-3 border border-gray-200 rounded-lg hover:bg-gray-50">
                                            <div>
                                                <p className="font-semibold text-gray-800">{member.fullName}</p>
                                                <p className="text-sm text-gray-500">{member.studentCode} • {member.email}</p>
                                            </div>
                                            <div className="text-right">
                                                <p className="text-sm font-medium text-gray-600">{member.role}</p>
                                                <p className="text-xs text-gray-500">Tham gia: {new Date(member.joinedAt).toLocaleDateString('vi-VN')}</p>
                                            </div>
                                        </div>
                                    ))
                                ) : (
                                    ...
                                    {joinRequests.map((request) => (
                                        <div key={request.id} className="p-4 border border-yellow-200 bg-yellow-50 rounded-lg">
                                            <div className="flex items-start justify-between mb-3">
                                                ...
                                                <span className="text-xs text-gray-500">{new Date(request.createdAt).toLocaleDateString('vi-VN')}</span>
                                            </div>
