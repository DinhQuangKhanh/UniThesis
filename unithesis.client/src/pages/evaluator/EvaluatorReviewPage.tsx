            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">
                  <span className="font-medium">Nguyễn Văn A</span> • K15 • GVHD: TS. Emily Tran
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <button
                onClick={() => navigate("/evaluator/similarity")}
                className="flex items-center gap-2 h-10 px-4 rounded-lg border border-gray-200 bg-white text-slate-700 text-sm font-semibold hover:bg-gray-50"
              >
          ...
          <div className="flex flex-col md:flex-row gap-6">
            <div className="flex-1">
              ...
              <p className="text-sm text-slate-700">
                Hệ thống điều khiển giao thông thông minh sử dụng AI để tối ưu hóa luồng giao thông tại các nút giao
                thông phức tạp.
              </p>
            </div>
            <div className="md:w-1/3">
              ...
              <ul className="text-sm text-slate-700 space-y-1">
                <li className="flex items-start gap-2">
                  <span className="material-symbols-outlined text-primary text-[16px]">check_circle</span>Phát triển
                  thuật toán AI
                </li>
                <li className="flex items-start gap-2">
                  <span className="material-symbols-outlined text-primary text-[16px]">check_circle</span>Mô phỏng và
                  đánh giá
                </li>
                <li className="flex items-start gap-2">
                  <span className="material-symbols-outlined text-primary text-[16px]">check_circle</span>Tích hợp hệ
                  thống
                </li>
              </ul>
            </div>
            ...
            <div className="bg-gray-50 border-b border-gray-200 px-4 py-3 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <button className="size-8 rounded-lg hover:bg-gray-200 flex items-center justify-center">
                  <span className="material-symbols-outlined text-slate-600 text-[20px]">remove</span>
                </button>
                <span className="text-sm font-medium text-slate-600 min-w-[4rem] text-center">100%</span>
                <button className="size-8 rounded-lg hover:bg-gray-200 flex items-center justify-center">
                  <span className="material-symbols-outlined text-slate-600 text-[20px]">add</span>
                </button>
                <div className="h-6 w-px bg-gray-300 mx-2" />
                <span className="text-sm text-slate-500">
                  Trang <span className="font-medium text-slate-900">1</span> /{" "}
                  <span className="font-medium text-slate-900">24</span>
                </span>
              </div>
              <div className="flex items-center gap-2">
                <button className="size-8 rounded-lg hover:bg-gray-200 flex items-center justify-center">
                  <span className="material-symbols-outlined text-slate-600 text-[20px]">fullscreen</span>
                </button>
                <button className="size-8 rounded-lg hover:bg-gray-200 flex items-center justify-center">
                  <span className="material-symbols-outlined text-slate-600 text-[20px]">print</span>
                </button>
              </div>
            </div>
      ...
      >
        <div className="px-6 py-5 border-b border-gray-200">
          <h2 className="text-lg font-bold text-slate-900 flex items-center gap-2">
            <span className="material-symbols-outlined text-primary">rate_review</span>Thẩm định đề tài
          </h2>
        ...
        <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-6">
          <div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {[
                { v: "approved", l: "Duyệt", c: "green", i: "check_circle" },
                { v: "revision", l: "Chỉnh sửa", c: "amber", i: "edit_note" },
                { v: "rejected", l: "Từ chối", c: "red", i: "cancel" },
              ].map(({ v, l, c, i }) => (
                <button
                  key={v}
                  onClick={() => setVerdict(v)}
                  className={`flex flex-col items-center justify-center p-4 rounded-xl border-2 transition-all ${verdict === v ? `border-${c}-500 bg-${c}-50` : `border-gray-200 hover:border-${c}-300`}`}
                >
                  <span
                    className={`material-symbols-outlined text-2xl mb-1 ${verdict === v ? `text-${c}-600` : "text-gray-400"}`}
                  >
                    {i}
                  </span>
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full h-32 px-4 py-3 rounded-xl border border-gray-200 bg-gray-50 text-sm resize-none focus:ring-2 focus:ring-primary/20 focus:border-primary focus:bg-white outline-none"
          ...
          <div>
            ...
            <div className="relative pl-4 space-y-3 before:absolute before:left-[7px] before:top-2 before:bottom-2 before:w-0.5 before:bg-gray-200">
              {[
                { v: "v1.2", d: "24/10/2023", s: "Hiện tại", c: true },
                { v: "v1.1", d: "22/10/2023", s: "Đã sửa", c: false },
                { v: "v1.0", d: "20/10/2023", s: "Ban đầu", c: false },
              ].map((r) => (
                <div key={r.v} className="relative pl-6">
                  <div
                    className={`absolute left-0 top-1 size-3 rounded-full border-2 ${r.c ? "bg-primary border-primary" : "bg-white border-gray-300"}`}
                  />
                  <div className="flex items-center justify-between">
                    ...
                    <span
                      className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${r.c ? "bg-green-50 text-green-600" : "bg-gray-100 text-gray-600"}`}
                    >
                      {r.s}
                    </span>
                  </div>
        ...
        <div className="px-6 py-4 border-t border-gray-200 flex gap-3">
          <button className="flex-1 h-11 rounded-xl border border-gray-200 text-slate-700 font-semibold text-sm hover:bg-gray-50">
            Lưu nháp
          </button>
          <button
            disabled={!verdict}
            className="flex-1 h-11 rounded-xl bg-primary text-white font-semibold text-sm hover:bg-primary-dark shadow-lg shadow-primary/20 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            Gửi thẩm định
          </button>
        </div>
            ...
            >
              <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-slate-900">Phân tích trùng lặp</h3>
                    <p className="text-xs text-slate-500">Kết quả kiểm tra sáo chép</p>
                  </div>
                </div>
                <button
                  onClick={() => setShowModal(false)}
              ...
              <div className="p-6 space-y-4">
                <div className="flex items-center gap-4 p-4 bg-gray-50 rounded-xl">
                  ...
                  <div>
                    <p className="text-sm font-bold text-green-600 mb-1">Mức độ tương đồng thấp</p>
                    <p className="text-xs text-slate-500">
                      Tài liệu có mức độ tương đồng 18%, đây là mức chấp nhận được.
                    </p>
                  </div>
                </div>
                <div className="space-y-2">
                  {[
                    { n: "IEEE Paper: AI Traffic", s: "ieee.org", p: "8%", c: "amber" },
                    { n: "Thesis 2022: Smart City", s: "Internal", p: "6%", c: "green" },
                    { n: "Wikipedia: Traffic", s: "wikipedia.org", p: "4%", c: "green" },
                  ].map((i) => (
                    <div key={i.n} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm font-medium text-slate-900">{i.n}</p>
              ...
              <div className="px-6 py-4 border-t border-gray-200 flex justify-end">
                <button
                  onClick={() => setShowModal(false)}
                  className="h-10 px-6 rounded-lg bg-primary text-white font-semibold text-sm hover:bg-primary-dark"
                >
                  Đóng
                </button>
              </div>
