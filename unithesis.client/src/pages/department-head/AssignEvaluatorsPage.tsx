...
export function AssignEvaluatorsPage() {
  ...
  return (
    <div className="p-8">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-slate-900">Phân công thẩm định</h1>
        <p className="text-slate-500 mt-1">Phân công evaluator và theo dõi kết quả thẩm định đề tài</p>
      </div>
        ...
        <div className="p-6">
          {loading ? (
            ...
            <AnimatePresence mode="wait">
              ...
              >
                {activeTab === "pending" && (
                  <PendingTab projects={getTabData("pending")} onAssign={setAssignModal} />
                )}
                {activeTab === "in-evaluation" && (
                  <InEvaluationTab projects={getTabData("in-evaluation")} />
                )}
                {activeTab === "needs-decision" && (
                  <NeedsDecisionTab projects={getTabData("needs-decision")} onDecide={setDecisionModal} />
                )}
                {activeTab === "completed" && (
                  <CompletedTab projects={getTabData("completed")} />
                )}
              </motion.div>
            </AnimatePresence>
          )}
