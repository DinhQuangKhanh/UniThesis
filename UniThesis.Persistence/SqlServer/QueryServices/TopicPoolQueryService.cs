...
/// <summary>
/// EF Core implementation of TopicPool read queries.
/// Uses AsNoTracking with direct projections for optimal performance.
/// </summary>
public class TopicPoolQueryService : ITopicPoolQueryService
{
    ...
    public async Task<List<TopicPoolDto>> GetTopicPoolsAsync(
        int? majorId,
    ...
    public async Task<TopicPoolDto?> GetTopicPoolByIdAsync(
        Guid id,
        CancellationToken cancellationToken = default)
    {
    ...
    public async Task<TopicPoolStatisticsDto> GetTopicPoolStatisticsAsync(
        Guid poolId,
        CancellationToken cancellationToken = default)
    {
        var pool = await _context.TopicPools
            ...
            .Select(tp => new { tp.Code, tp.Name })
            .FirstOrDefaultAsync(cancellationToken);

        if (pool is null)
            return new TopicPoolStatisticsDto { PoolId = poolId };
